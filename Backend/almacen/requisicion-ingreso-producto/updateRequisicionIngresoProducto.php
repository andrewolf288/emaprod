...
if ($_SERVER["REQUEST_METHOD"] == "PUT") {
    ...
    if ($pdo) {
        $fecActReqIngProd = date('Y-m-d H:i:s'); // fecha de actualizacion
        $sql_update_detalle_requisicion_ingreso_producto =
            "UPDATE produccion_ingreso_producto SET canProdIng = $canReqIngDetNew, fecActProdIng = ?
            WHERE id = ?";

        try {
            $stmt_update_detalle_requisicion_ingreso_producto = $pdo->prepare($sql_update_detalle_requisicion_ingreso_producto);
            $stmt_update_detalle_requisicion_ingreso_producto->bindParam(1, $fecActReqIngProd);
