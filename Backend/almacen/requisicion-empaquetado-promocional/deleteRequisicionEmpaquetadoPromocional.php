...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $idReqEmpProm = $data["idReqEmpProm"]; // requisicion agregacion
    $idReqEmpPromDet = $data["id"]; // requisicion agregacion detalle

    if ($pdo) {

        $sql_delete_detalle_requisicion_empaquetado_promocional =
            "DELETE FROM requisicion_empaquetado_promocional_detalle
            WHERE id = ?";
        try {
            ...
            $stmt_delete_detalle_requisicion_empaquetado_promocional->bindParam(1, $idReqEmpPromDet, PDO::PARAM_INT);
        ...
        if (empty($message_error)) {
            try {
                ...
                $stmt_consulta_requisicion_detalle->bindParam(1, $idReqEmpProm, PDO::PARAM_INT);
                $stmt_consulta_requisicion_detalle->bindParam(2, $esCom, PDO::PARAM_BOOL);
                ...
                if ($idReqEst == 3) {
                    ...
                    $stmt_update_requisicion_completo->bindParam(2, $fecEntReqEmpProm);
