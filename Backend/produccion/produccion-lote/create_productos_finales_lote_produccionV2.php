...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    //regProFin
    if ($pdo) {
        // primero debemos verificar si las salidas de requisicion se han realizado
        // buscamos si existen requisiciones no completadas
        $idReqEst = 3;
        $sql_count_requisiciones_completas =
            "SELECT * FROM requisicion 
        WHERE idProdc = ? AND idReqEst <> ? LIMIT 1";
        $stmt_count_requisiciones_completas = $pdo->prepare($sql_count_requisiciones_completas);
        $stmt_count_requisiciones_completas->bindParam(1, $idProdc, PDO::PARAM_INT);
        $stmt_count_requisiciones_completas->bindParam(2, $idReqEst, PDO::PARAM_INT);
        $stmt_count_requisiciones_completas->execute();

        if ($stmt_count_requisiciones_completas->rowCount() == 0) {
            ...
            $message_error = "Requisición no completa";
            $description_error = "Las requisiciones no fueron atendidas por el área de almacén";
        }
    } else {
