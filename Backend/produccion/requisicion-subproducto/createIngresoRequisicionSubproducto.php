    ...
    if ($pdo) {
        try {
            $sql_create_ingreso_requisicion_subproducto =
                "INSERT INTO requisicion_ingreso_subproducto
            (idReq, idProdt, canProdIng, fecProdIng, fecProdVen)
            VALUES(?, ?, $canProdIng, ?, ?)";
            $stmt_create_ingreso_requisicion_subproducto = $pdo->prepare($sql_create_ingreso_requisicion_subproducto);
            $stmt_create_ingreso_requisicion_subproducto->bindParam(1, $idReq, PDO::PARAM_INT);
