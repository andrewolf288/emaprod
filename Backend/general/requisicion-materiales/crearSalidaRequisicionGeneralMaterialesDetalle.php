...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $canReqMatDet = floatval($data["canReqMatDet"]); // cantidad de requisicion detalle
    ...
    if ($pdo) {
        ...
        if (empty($message_error)) {
            ...
            $row_almacen_destino = $stmt_consult_almacen_destino->fetch(PDO::FETCH_ASSOC);
            $idAlmDes = $row_almacen_destino["id"];
            ...
            try {
                ...
                foreach ($entradasUtilizadas as $item) {
                    ...
                    $sql =
                        "INSERT
                    salida_requisicion_materiales
                    (idReqMat, idReqMatDet, canSalReqMatDet, idEntSto, idAlmDes)
                    VALUES (?, ?, $canSalStoReq, ?, ?)";

                    $stmt = $pdo->prepare($sql);
                    $stmt->bindParam(1, $idReqMat, PDO::PARAM_INT);
                    $stmt->bindParam(2, $idReqMatDet, PDO::PARAM_INT);
                    $stmt->bindParam(3, $idEntSto, PDO::PARAM_INT);
            ...
            if (empty($message_error)) {
                try {
                    ...
                    $stmt_consulta_requisicion_materiales_detalle->bindParam(1, $idReqMat, PDO::PARAM_INT);
                    $stmt_consulta_requisicion_materiales_detalle->bindParam(2, $fueCom, PDO::PARAM_BOOL);
                    ...
                    $idReqMatEst = 0; // inicializacion

                    if ($total_requisiciones_detalle_no_completadas == 1) { // si es la unica requisicion detalle por completar
                        $idReqMatEst = 3; // COMPLETADO
                    } else {
                        $idReqMatEst = 2; // EN PROCESO
                    }

                    // PRIMERO ACTUALIZAMOS EL DETALLE
                    $sql_update_requisicion_materiales_detalle =
                        "UPDATE requisicion_materiales_detalle
                    SET fueCom = ?
                    WHERE id = ?";
                    $stmt_update_requisicion_materiales_detalle = $pdo->prepare($sql_update_requisicion_materiales_detalle);
                    $stmt_update_requisicion_materiales_detalle->bindParam(1, $fueCom, PDO::PARAM_BOOL);
                    $stmt_update_requisicion_materiales_detalle->bindParam(2, $idReqMatDet, PDO::PARAM_INT);
                    ...
                    if ($idReqMatEst == 3) {
                        ...
                        $stmt_update_requisicion_completo->bindParam(1, $idReqMatEst, PDO::PARAM_INT);
                        ...
                        $stmt_update_requisicion->bindParam(1, $idReqMatEst, PDO::PARAM_INT);
