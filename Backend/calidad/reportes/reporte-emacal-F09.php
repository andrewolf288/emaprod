...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($categoria == 1) {
        ...
        $sql_select_producto =
            "SELECT p.id
        FROM producto AS p
        WHERE p.idSubCla = ? LIMIT 1";
        $stmt_select_producto = $pdo->prepare($sql_select_producto);
        $stmt_select_producto->bindParam(1, $idSubCla, PDO::PARAM_INT);
        $stmt_select_producto->execute();
        $result_select_producto = $stmt_select_producto->fetch(PDO::FETCH_ASSOC);
        $producto = $result_select_producto["id"];
    }
    ...
    if ($categoria == 3) {
        ...
        $stmt_entradas_calidad = $pdo->prepare($sql_entradas_calidad);
        $stmt_entradas_calidad->bindParam(1, $isSubCla, PDO::PARAM_INT);
        $stmt_entradas_calidad->bindParam(2, $isSubCla2, PDO::PARAM_INT);
        $stmt_entradas_calidad->bindParam(3, $isSubCla3, PDO::PARAM_INT);
        $stmt_entradas_calidad->bindParam(4, $isSubCla4, PDO::PARAM_INT);
        $stmt_entradas_calidad->execute();
        $entradas_calidad = $stmt_entradas_calidad->fetchAll(PDO::FETCH_ASSOC);
    }
    ...
    if ($categoria == 4) {
        ...
        $stmt_entradas_calidad->bindParam(3, $isSubCla3, PDO::PARAM_INT);
        $stmt_entradas_calidad->bindParam(4, $isSubCla4, PDO::PARAM_INT);
        $stmt_entradas_calidad->bindParam(5, $isSubCla5, PDO::PARAM_INT);
        $stmt_entradas_calidad->execute();
        $entradas_calidad = $stmt_entradas_calidad->fetchAll(PDO::FETCH_ASSOC);
    }
    ...
    if ($categoria == 5) {
        ...
        $stmt_entradas_calidad->bindParam(4, $isSubCla4, PDO::PARAM_INT);
        $stmt_entradas_calidad->bindParam(5, $isSubCla5, PDO::PARAM_INT);
        $stmt_entradas_calidad->bindParam(6, $isSubCla6, PDO::PARAM_INT);
        $stmt_entradas_calidad->bindParam(7, $isSubCla7, PDO::PARAM_INT);
        $stmt_entradas_calidad->bindParam(8, $isSubCla8, PDO::PARAM_INT);
        $stmt_entradas_calidad->execute();
        $entradas_calidad = $stmt_entradas_calidad->fetchAll(PDO::FETCH_ASSOC);
    }
    ...
    if ($categoria == 6) {
        ...
        $stmt_entradas_calidad->bindParam(2, $idProdt2, PDO::PARAM_INT);
        $stmt_entradas_calidad->bindParam(3, $idProdt3, PDO::PARAM_INT);
        $stmt_entradas_calidad->execute();
        $entradas_calidad = $stmt_entradas_calidad->fetchAll(PDO::FETCH_ASSOC);
    }
    ...
    foreach ($entradas_calidad as $row_entradas_calidad) {
